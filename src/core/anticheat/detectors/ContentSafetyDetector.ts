...
身体動作を表す語にも慣用的・比喩的な用法があります。辞書上または想像上の性的用法を列挙せず、その投稿で最も自然な意味を採用してください。特に「俺のこと舐めてる？／舐めてるやろ」は「見下している・軽く見ている」という挑発、「うまそう／舐めたい」は食べ物や物への通常表現になり得るため、それだけならsuggestive=0です。「舐める」「触る」「濡れる」「イく」などの語だけを根拠にしません。性的な身体部位、性的対象、性行為、明確な性的誘いなど、投稿内で確認できる別の証拠が組み合わさった場合に限り性的カテゴリへ加点します。根拠を一意に説明できなければ0にし、explanationやpointsReasonにも仮定した性的解釈を書きません。
...
violenceは、人や動物などの生き物に対する身体的な攻撃、負傷、殺傷、流血、損壊を描写・推奨する表現として判定します。物体、素材、建造物、ゲーム内オブジェクトなどを「壊す」「砕く」「潰す」「切る」と述べただけではviolenceに加点しません。たとえば「樹脂砕いちゃえよ」「氷を砕く」「箱を壊す」「岩を爆破する」は、対象が生き物または人の身体だと文脈から確認できない限りviolence=0です。「破壊しろ」という命令形や強い語調だけを根拠に、暴力的なニュアンスを推測しません。比喩、制作・加工、解体、廃棄、清掃、料理、採掘、ゲームなど、会話上自然な非暴力的文脈を優先してください。

加点も同じ基準に従います。対象が物体である表現、対象が不明な表現、または語調が荒いだけの表現には、暴力を理由とするsuggestedPointsを付けません。誰に何をする表現なのかを投稿と会話文脈から確認し、生き物への危害を一意に説明できる場合だけviolenceへ加点し、pointsReasonには実際に確認できた対象と行為を書いてください。
...
violenceは人や動物などの生き物への身体的危害、負傷、殺傷、流血を対象とします。物体・素材・建造物・ゲーム内オブジェクトを壊す、砕く、潰すという表現だけでは加点しません。「樹脂砕いちゃえよ」「氷を砕く」「箱を壊す」などは、対象が生き物だと文脈から確認できない限りviolence=0です。命令形、荒い語調、「破壊」のような単語だけで暴力を推測せず、加工・解体・廃棄・料理・採掘・ゲームなどの自然な文脈を確認してください。対象が不明で、生き物への危害を一意に説明できない場合もviolence=0とし、暴力を理由に加点しません。
通常文を出力せずsubmit_verdictを正確に1回だけ呼び出してください。各値はツール引数へ直接入れます。
explanationは確認した事実を80文字以内の日本語1文で記載せよ`;
