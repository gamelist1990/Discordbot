...
test('履歴を現在入力や最新画像として扱わない指示を付与する', () => {
    const history = buildConversationHistory({
        dataset: [{
            id: 'old-image-conversation',
            timestamp: 400,
            channelId: 'channel-a',
            userId: 'user-a',
            userName: 'Alice',
    ...
    const instruction = String(history[0]?.content);
    assert.match(instruction, /過去の会話であり、現在の入力ではありません/);
    assert.match(instruction, /最新画像として扱わない/);
    assert.match(instruction, /最後のuserメッセージ/);
});
