...
test('strictModel指定時はPEXフォールバックモデルを候補へ追加しない', async () => {
    const manager = new (await import('../src/core/ai/OpenAIChatManager.ts')).OpenAIChatManager({
        apiEndpoint: 'http://127.0.0.1:1',
        apiKey: 'test-key',
        defaultModel: config.pexAi.model,
    });

    const options = (manager as any).withPexModelFallback({
        model: config.pexAi.model,
        strictModel: true,
        requestLabel: 'chat-ai-channel-memory',
    });

    assert.equal(options.model, config.pexAi.model);
    assert.equal(options.strictModel, true);
    assert.equal(options.fallbackOnLimitOnly, undefined);
});

test('chat-ai-channelはprimaryとfallbackだけを制限時フォールバック候補にする', async () => {
    const { resolveChatAIChannelModels } = await import('../src/core/ChatAIChannel/ChatAIChannelManager.ts');

    assert.deepEqual(
        resolveChatAIChannelModels('gemini-3.1-flash-lite', 'fallback-model'),
        ['gemini-3.1-flash-lite', 'fallback-model'],
    );
    assert.deepEqual(
        resolveChatAIChannelModels('gemini-3.1-flash-lite', 'gemini-3.1-flash-lite'),
        ['gemini-3.1-flash-lite'],
    );
    assert.deepEqual(
        resolveChatAIChannelModels('gemini-3.1-flash-lite', ''),
        ['gemini-3.1-flash-lite'],
    );
});